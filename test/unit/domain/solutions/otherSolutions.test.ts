...
} from '../../../../src/domain/solutions/otherSolutions';
...
describe('other solution rules', () => {
  ...
  it('prefers the configured language and avoids an immediate repeat', () => {
    ...
    expect(selectRandomOtherSolution(files, 'CaseUser', 'py', undefined, () => 0)).toBe(
      'FirstPython.py',
    );
    expect(selectRandomOtherSolution(files, 'CaseUser', 'py', 'FirstPython.py', () => 0.99)).toBe(
      'SecondPython.py',
    );
  });
