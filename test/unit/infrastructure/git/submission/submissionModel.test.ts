...
import type { SubmissionFileSnapshot } from '../../../../../src/shared/contracts';
import {
  ...
  weekBranchName,
  weekFromBranch,
} from '../../../../../src/infrastructure/git/submission/submissionModel';
import type { RemoteSubmissionState } from '../../../../../src/infrastructure/github/githubSubmissionClient';
...
describe('submission status projection', () => {
  ...
  it('keeps local conflict and index states ahead of remote progress', () => {
    ...
    const files = [conflicted, outdated, staged, working, pending, open, merged];
    const indexPaths = new Set([
      conflicted.relativePath,
      outdated.relativePath,
      staged.relativePath,
    ]);
    const workingPaths = new Set([outdated.relativePath, working.relativePath]);
