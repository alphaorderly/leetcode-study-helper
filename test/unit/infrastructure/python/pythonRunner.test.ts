...
const dataset = JSON.parse(readFileSync('resources/leetcode-python-tests.json', 'utf8')) as {
  problems: Record<string, DatasetProblem>;
};

function runner(request: Record<string, unknown>): Record<string, unknown> {
  const result = spawnSync('python3', ['-I', '-u', 'resources/python/leetcode_runner.py'], {
