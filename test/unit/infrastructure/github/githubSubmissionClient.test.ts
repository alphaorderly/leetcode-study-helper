...
} from '../../../../src/infrastructure/github/githubSubmissionClient';
...
describe('GitHubSubmissionClient branch state', () => {
  it('loads every page of open PRs and PR files', async () => {
        ...
        if (url.includes('/pulls/101/files') && page === '1') {
          return response(
            Array.from({ length: 100 }, (_, index) => ({
              filename: `problem-${index + 1}/CaseUser.py`,
            })),
          );
        }
    ...
    expect(
      requests.some((url) => url.includes('/pulls?state=open') && url.includes('page=2')),
    ).toBe(true);
    expect(requests.some((url) => url.includes('/pulls/101/files') && url.includes('page=2'))).toBe(
      true,
    );
    client.dispose();
  });
...
describe('GitHubSubmissionClient authentication', () => {
  ...
  it('throws a sign-in error for unauthenticated 403 compare requests', async () => {
    ...
    await expect(client.getRemoteSubmission(remote, 'week-01', true)).rejects.toMatchObject({
