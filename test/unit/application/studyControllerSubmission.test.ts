...
describe('StudyController weekly submission guard', () => {
  it('blocks staging a different week while an open PR is active', async () => {
    ...
    await expect(controller.stageSolution('file:///study/three-sum/CaseUser.py')).rejects.toThrow(
  ...
  it('delegates the fresh push-range guard to the Git service', async () => {
    ...
    await expect(controller.pushActiveWeek('file:///study')).rejects.toThrow(
      'Week 1 제출이 끝나기 전에는 Week 2',
    );
  ...
  it('blocks committing a file changed after it was staged', async () => {
    ...
    await expect(
      controller.commitActiveWeek('file:///study', '[CaseUser] WEEK 01 Solutions'),
