import type { LeetCodeProblemDetail, ProblemTopicTag } from '../../shared/contracts';
...
/** 문제 조회 요청에 주입할 표준 fetch 함수의 계약입니다. */
type Fetch = typeof fetch;

/** 응답 필드 검사에 앞서 값이 null이 아닌 객체인지 확인합니다. */
function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === 'object' && value !== null;
}

/** 문제 태그 배열의 이름·slug를 검증하며 형식이 다르면 오류를 던집니다. */
...
/** 문제 응답 필드를 검증하고 null 설명을 undefined로 변환합니다. */
...
export class LeetCodeApiService {
  ...
  /** HTTP 요청 구현과 문제 조회 시간 제한을 보관합니다. */
  constructor(
  ...
  /**
   * 진행 중 요청과 성공 결과를 재사용하며 실패한 요청은 캐시에서 제거해 재시도를 허용합니다.
   * @throws HTTP·네트워크·시간 제한 오류 또는 잘못된 응답 형식인 경우.
   */
  ...
  /** GraphQL로 문제를 읽고 네트워크·HTTP·응답 형식 실패를 안내 오류로 전달합니다. */
  private async fetchProblem(slug: string): Promise<LeetCodeProblemDetail> {
    ...
    try {
      ...
      throw new Error('LeetCode 문제를 불러오지 못했습니다. 네트워크 연결을 확인해 주세요.', {
        cause: error,
      });
    }
