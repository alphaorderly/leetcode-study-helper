...
공식 저장소 remote 이름은 `upstream`이 아니어도 됩니다. fetch 또는 push URL이 `DaleStudy/leetcode-study`이면 그 remote를 사용합니다. 그런 remote가 없으면 **첫 주차 제출**이나 제출 탭의 **지금 맞추기**에서 `upstream`으로 `https://github.com/DaleStudy/leetcode-study.git`을 추가합니다. 이때 merge는 하지 않습니다. 포크가 공식 `main`보다 뒤처져 있으면 **포크 동기화**로 맞춥니다. 이름이 `upstream`인데 다른 저장소를 가리키면 안전을 위해 동기화를 중단합니다.
...
제출 기능은 GitHub에서 `DaleStudy/leetcode-study`의 포크로 확인된 저장소에서 동작합니다. 새 주차 브랜치는 공식 `main` tip에서 만들고, 커밋 이후에는 해당 `week-XX` 브랜치에서 이어집니다. 포크 `main`이 공식 저장소보다 앞서 있어도(GitHub Sync merge 등) 주차 브랜치의 기준은 공식 `main`입니다. 공식 remote가 없거나 포크가 뒤처져 있으면 제출 탭에 **지금 맞추기**가 나타납니다.
...
공식 저장소의 변경을 포크에 반영하려면 **제출** 탭의 **포크 동기화** 또는 **지금 맞추기**를 사용합니다.

`main`이 origin과 분기되지 않았다면, 문제 카드에서 **커밋에 추가**할 때도 같은 동기화를 먼저 실행한 뒤 스테이징을 이어갑니다. GitHub 웹의 Sync fork를 누르지 않아도 됩니다. 내 풀이 파일만 수정된 경우에도 동기화를 시도합니다. 공식 `main`이 같은 파일을 바꾸면 충돌로 중단하고 push하지 않습니다.

풀이 외 추적 파일이 수정되어 있으면 제출 탭에 경로가 나타나고 **풀이 외 변경 되돌리기**로 해당 파일만 HEAD로 되돌릴 수 있습니다. 스테이징된 풀이는 문제 카드에서 커밋에 추가를 해제한 뒤 동기화합니다.
...
- 현재 브랜치가 `main`
- 스테이징된 변경 없음
- 풀이 외 추적 파일의 작업 트리 변경 없음 (내 풀이 수정과 untracked 풀이는 막지 않습니다)
...
| `풀이 외 파일이 스테이징되어 있습니다` | 제출 탭의 **풀이 외 변경 되돌리기** 또는 문제 카드의 커밋에 추가 해제를 사용합니다. |
...
| `풀이 외 추적 파일 변경을 되돌린 뒤 포크를 동기화` | 제출 탭에 표시된 풀이 외 파일을 되돌린 뒤 **지금 맞추기**를 누릅니다. |
| `스테이징된 파일을 먼저 정리해 주세요` | 스테이징된 풀이는 문제 카드에서 커밋에 추가를 해제합니다. |
| 포크 동기화 버튼이 비활성화됨 | `main`이 아닌 브랜치, 스테이징, 풀이 외 추적 파일 수정, origin보다 앞선 로컬 커밋, merge·rebase 상태를 확인합니다. 제출 탭 본문에 막는 파일과 이유가 표시됩니다. 내 풀이만 수정되었거나 untracked 풀이만 있는 경우에는 막히지 않습니다. |
