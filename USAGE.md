...
제출 기능은 GitHub에서 `DaleStudy/leetcode-study`의 포크로 확인된 저장소와 `main` 브랜치에서만 동작합니다.
...
메시지를 확인하고 **이 주차 커밋**을 누릅니다. 커밋 직전에는 실제 Git index가 화면의 풀이 목록과 정확히 일치하는지 다시 검사합니다.

### 3단계: origin에 push

로컬 커밋이 생기면 `origin/main` 노드에 **origin에 push** 버튼이 표시됩니다. push 전에는 다음 조건을 다시 확인합니다.

- 현재 브랜치가 `main`인지
- `origin/main..HEAD`의 커밋 수가 안전한 범위인지
...
소스: study-user/leetcode-study:main
```
...
충돌이 발생하면 진행 중인 merge를 중단하고 원래 상태로 돌아갑니다. rebase, force push, 자동 stash는 사용하지 않습니다. 추적되지 않은 새 풀이 파일은 자동 stash하거나 삭제하지 않습니다.
...
| 포크 동기화 버튼이 비활성화됨 | 스테이징, 추적 파일 수정, 미푸시 커밋, merge·rebase 상태를 먼저 정리합니다. |
