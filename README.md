...
주석은 처음 읽는 사람이 다음을 판단할 수 있도록 작성합니다.

- 클래스: 누가 사용하고, 어떤 상태를 소유하며, 언제 구독·타이머·프로세스를 정리하는가?
- 비동기 작업: 중복 요청을 공유하는가, 다음 실행으로 모으는가? 취소 후 결과는 어떻게 되는가?
- 파일·Git 작업: 읽기만 하는가, 저장·브랜치 전환·push도 하는가? 실패하면 무엇이 남는가?
- 상태·반환값: `undefined`는 대상 없음인가, 조회 실패인가? 오류를 던지는가, 표시 상태로 반환하는가?

`@param`에는 타입만으로 알 수 없는 옵션·단위·전제조건을, `@returns`에는 취소나 대체값의
의미를, `@throws`에는 호출자가 처리해야 할 실패를 설명합니다. 단순한 순수 함수는 짧게
설명하되, 복잡한 작업은 단계와 그 순서를 지켜야 하는 이유까지 적습니다.
예를 들어 “Git 상태를 확인합니다”보다 “원격 조회 중 바뀐 HEAD와 origin을 쓰기 직전에
다시 확인합니다. 앞선 검증을 재사용하면 다른 커밋이나 저장소에 쓸 수 있습니다”가 유용합니다.
...
| 웹뷰의 `state/`, `views/` | 화면 표시 조건·문구 계산과 단계별 DOM 생성 |
...
1. `src/extension.ts`에서 확장 활성화와 명령·뷰 등록을 확인합니다.
2. `src/shared/contracts.ts`에서 화면 상태와 양방향 메시지를 읽습니다.
3. `StudyController`에서 관심 있는 사용자 명령을 찾습니다. 파일 탐색·갱신은
   `RepositoryRefreshSession`, 현재 풀이·실행은 `CurrentProblemSession`으로 이어집니다.
4. Git 제출은 `SubmissionActions`의 공개 메서드에서 시작해 검증·브랜치 작업으로 내려갑니다.
   표시가 이상하면 `SubmissionStatusReader`와 `submissionSnapshot`부터 봅니다.
5. 화면은 `webview/main.ts` → `render.ts` → 해당 `views/` 순서로 읽습니다.
   버튼 비활성 조건과 표시 문구는 대응하는 `state/` 모델에 있습니다.

확장 호스트에서 `application`, `infrastructure`, `webview/host`가 실행됩니다.
브라우저에서 실행되는 나머지 웹뷰는 VS Code API를 직접 호출하지 않고 메시지를 보냅니다.
Python 실행기는 별도 프로세스이며 stdin/stdout의 JSON으로 확장 호스트와 통신합니다.
...
```mermaid
flowchart TD
  A[확장 활성화와 웹뷰 ready] --> B[StudyController.getState]
  B --> C[RepositoryRefreshSession 전체 탐색]
  C --> D[파일 목록 먼저 게시]
  D --> E[웹뷰 state 메시지와 목록 렌더]
  C --> F[Git 후속 조회]
  F --> G{조회 기준 배열이 현재 배열인가?}
  G -->|예| H[Git 상태 결합 후 게시]
  G -->|아니오| I[옵션 복구 후 최신 목록 재조회]
  I --> F
  H --> E
```

전체 탐색 중 다시 요청하면 진행 중인 Promise를 공유합니다. 문제별 탐색은 루트·slug별로
공유합니다. Git 조회는 실행 중 들어온 요청과 강제 조회 옵션을 누적해 다음 회차까지 처리합니다.
이 세 가지 정책은 같지 않습니다. 갱신 결과는 새 스냅샷으로 게시하며, 배열 참조는 진행 중
Git 조회가 오래된 기준을 사용했는지 판별하는 데도 쓰입니다.

**현재 풀이의 분석과 실행**

```mermaid
flowchart TD
  A[편집기 선택 또는 문서 변경] --> B[기존 분석 취소와 필요한 실행 취소]
  B --> C[마지막 변경 350ms 후 Python 분석]
  C --> D[후보와 실행 가능 상태 게시]
  D --> E[사용자가 후보 실행]
  E --> F[테스트 데이터와 현재 소스 준비]
  F --> G[Python 실행]
  G --> H{취소되지 않았고 같은 파일인가?}
  H -->|예| I[실행 결과 게시]
  H -->|아니오| J[이전 결과 폐기]
```

문제 설명은 slug별로 보관하지만 분석·실행은 풀이 URI에 귀속됩니다. 결과 게시 조건과
제어기 정리 조건은 다릅니다. 이전 작업이 끝나도 새 작업의 AbortController를 지우면 안 됩니다.
입력 준비 실패는 명령 오류로 전달하고, 실행 중 실패는 후보를 보존한 러너 오류로 표시합니다.

**Git 제출**

```mermaid
flowchart TD
  A[웹뷰 제출 명령] --> B[컨트롤러의 화면 상태 사전 확인]
  B --> C[실제 Git 상태와 포크 신원 검증]
  C --> D[제출 범위와 주차 검증]
  D --> E[필요한 브랜치 전환 또는 원격 조회]
  E --> F[쓰기 직전 실제 상태 재검증]
  F --> G[commit 또는 push]
  G --> H[표시 상태 다시 조회]
```

위 그림은 공통 흐름입니다. 각 명령의 실제 조회·검증 순서는 `SubmissionActions`에 있습니다.
네트워크 조회와 브랜치 전환 사이에 상태가 바뀔 수 있으므로, 비슷해 보이는 검증도 실행 시점이
다르면 유지합니다. 포크 동기화 push가 실패하면 로컬에 남은 병합 결과를 보존하고 복구 안내를
반환합니다. 가독성 개선을 위해 검사나 Git 작업을 병렬화하지 않습니다.

### 표시 상태를 해석할 때

| 상태 | 의미와 주의점 |
| --- | --- |
| `checking` | 아직 조회·분석 중입니다. 이전 표시 데이터가 함께 남아 있을 수 있습니다. |
| `unknown` | 파일의 반영 상태를 판단할 근거가 없습니다. 미제출로 확정한 상태가 아닙니다. |
| `unavailable` | 조회 실패 또는 실행 불가입니다. Git 조회에서는 이미 확보한 로컬 이력을 보존할 수 있습니다. |
| `unsupported` | 지원하는 포크 등 필요한 대상 조건을 만족하지 않습니다. |
| `blocked` | 제출을 막는 사유가 계산되었습니다. `blockedReason`의 우선순위를 유지해야 합니다. |
| `ready` | 해당 단계의 데이터가 준비되었습니다. 화면의 ready는 Git 쓰기 승인을 뜻하지 않습니다. |

풀이의 `pushed`는 현재 upstream 기준이고, `merged`는 공식 저장소 반영 여부를 나타내므로
같은 뜻이 아닙니다. `completed`도 정답 통과가 아니라 내 풀이 파일 존재 여부입니다.
자세한 필드 의미와 선택적 값의 부재 조건은 `shared/contracts.ts`에 설명되어 있습니다.
