...
| 제출 | 선택한 풀이만 주차별로 스테이징하고 커밋, push, PR 진행 상태를 하나의 흐름으로 보여 줍니다. |
...
`작성 중 → 커밋 준비 → push 필요 → PR 진행 → 병합 완료` 상태를 문제 카드와 제출 탭에서 함께 확인합니다. 확장에서 직접 선택한 풀이만 커밋 대상으로 사용합니다.
...
- 안전한 상태에서 공식 `main`을 병합하고 포크의 `origin/main`에 push합니다.
